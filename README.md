...
- 将前面编译前端时所生成的dist文件夹复制到naslite.exe所在的目录中
- 运行naslite.exe
- 在浏览器中输入 http://127.0.0.1:8888 打开后台管理页面，默认账号admin 默认密码123456 登录之后进行完整的配置即可（这里使用了127.0.0.1来进入后台管理只是初步演示所用，想要完整的使用NasLite所有功能，必须要有域名才可以）
## Linux
- 待补充...
