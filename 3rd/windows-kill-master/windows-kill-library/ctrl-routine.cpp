...
namespace WindowsKillLibrary {
	...
	void CtrlRoutine::findAddressFromKernel32Dll() {
		if (this->address == NULL)
		{
			printf("address is null, prepare refind it from kernel32.dll \n");

			HMODULE hModule = GetModuleHandleA("kernel32.dll");
			if (hModule != NULL) {
				this->address = (LPTHREAD_START_ROUTINE)GetProcAddress(hModule, "CtrlRoutine");

				printf("get CtrlRoutine from kernel32.dll successed: %llu\n", std::size_t(this->address));
			}
			else {
				printf("get CtrlRoutine from kernel32.dll failed.\n");
			}
		}
	}
	...
	void CtrlRoutine::findAddressByStackBackTrace() {
		...
		if (count != 1) {
			//printf("Cannot capture stack back trace.\n");
			findAddressFromKernel32Dll();
			return;
			/*	TODO: Remove. No Exception Available here.
		...
		if (!SymInitialize(hProcess, NULL, TRUE)) {
			//printf("Cannot SymInitialize. Code: \n");
			findAddressFromKernel32Dll();
			return;
			/*	TODO: Remove. No Exception Available here.
		...
		if (!SymFromAddr(hProcess, (DWORD64)ctrlRoutine, &dwDisplacement, pSymbol)) {
			//printf("Cannot SymFromAddr. Code: \n");
			findAddressFromKernel32Dll();
			return;
			/*	TODO: Remove. No Exception Available here.
